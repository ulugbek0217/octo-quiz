...
func (app *App) Start(ctx context.Context, b *bot.Bot, u *models.Update) {
	...
	app.DashBoard(ctx, u.Message.From.ID)

}

func (app *App) DashBoard(ctx context.Context, chatID int64) {
	user, _ := app.isNewUser(chatID)
	kbd, err := builder.NewInlineKeyboardBuilder(builder.StudentMainMenuInlineButtons)
	if user.Role == "teacher" {
		kbd, err = builder.NewInlineKeyboardBuilder(builder.TeacherMainMenuInlineButtons)
	}
	if err != nil {
		log.Fatal(err)
	}
